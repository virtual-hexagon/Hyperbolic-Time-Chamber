...
class Double_Link_List {
    ...
    public:
        ...
        void insert_before(size_t index , value_type value){
            // Notes:
        ...
        void insert_after(size_t index , value_type value){
            // Notes:
            // - Check if we are trying to insert the node out of bounds
            // - Create pointers to track prior and current node as we iterate through list
            // - Create a temporary node to insert at the desired index.
            // - We will insert the new node in behind the "current" node
            // - Create a node that we plan to insert using manual memory allocation via `new`
            if(index > size){
                std::string msg = "Trying to insert at index beyond the end of the list. List Size: "
                    + std::to_string(size)
...
int main() {
    ...
    // lets insert something before the element in the middle of the list.
    auto pos = list.getSize()/2;
    std::cout << std::endl <<"Inserting data at position "<< pos <<" ..." << std::endl;
    list.insert_before( pos, 44 );
    std::cout << "The list size is now: " << list.getSize() << std::endl;
    std::cout << "The list data is: " << std::endl;
    list.display();

    // lets insert something after the element in the middle of the list.
    pos = list.getSize()/2;
    std::cout << std::endl <<"Inserting data at position "<< pos <<" ..." << std::endl;
    list.insert_after( pos, 44 );
    std::cout << "The list size is now: " << list.getSize() << std::endl;
    std::cout << "The list data is: " << std::endl;
    list.display();

    // lets insert something beyond the end of the list
    std::cout << std::endl << "Try to insert node in front of an index beyond the end of the list ..." << std::endl;
    pos = list.getSize()+2;
    try{
        list.insert_before(pos,666);
    ...
    // lets insert something beyond the end of the list
    std::cout << std::endl << "Try to insert node after an index beyond the end of the list ..." << std::endl;
    pos = list.getSize()+2;
    try{
        list.insert_after(pos,666);
    } catch ( const std::range_error& e ) {
        std::cout << "Caught the Range Error!" << std::endl;
        std::cout << e.what() << std::endl;
        std::cout << "Continuing ..." << std::endl;
